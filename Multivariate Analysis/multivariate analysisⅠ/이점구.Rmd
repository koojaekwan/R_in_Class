...
```{r}
data_long<-gather(data,key="category",value="record")

data_long %>% filter(category!="Marathon") %>% ggplot(aes(x=category,y=record)) +geom_boxplot()

data_long %>% filter(category=="Marathon") %>% ggplot(aes(x=category,y=record)) +
  geom_boxplot(outlier.size=3, outlier.shape="▲", outlier.colour="red")+
  stat_summary(fun.y="mean", geom="point", shape=21, size=3, fill="blue")
```  

```{r}
data_long %>% ggplot(aes(x=record))+
  geom_histogram()+
  facet_grid(category ~ .)
```  
